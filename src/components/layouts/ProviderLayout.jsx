...
const NAV_SECTIONS = [
  ...
  { label: 'Services', items: [{ to: '/provider/services', icon: Package, text: 'My Services' }, { to: '/provider/services/new', icon: Package, text: 'Add Service', highlight: true }] },
...
export default function ProviderLayout({ children }) {
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full">
      <div className={`flex items-center gap-3 px-4 py-5 border-b border-white/5 ${collapsed ? 'justify-center' : ''}`}>
        ...
        {collapsed && <Briefcase className="w-6 h-6 text-emerald-400" />}
      </div>
      ...
      <div className="p-3 border-t border-white/5">
        {!collapsed && (
          <div className="flex items-center gap-2 px-2 py-2 mb-2">
            <div className="w-7 h-7 rounded-full bg-emerald-700 flex items-center justify-center flex-shrink-0"><Briefcase className="w-3.5 h-3.5 text-white" /></div>
  ...
  return (
    <div className="flex h-screen bg-[#0a0a0a] text-white overflow-hidden">
      ...
      <button onClick={() => setCollapsed(v => !v)} className="hidden lg:flex absolute z-10 top-20 items-center justify-center w-5 h-5 rounded-full bg-emerald-700 text-white shadow-lg transition-all duration-300" style={{ left: collapsed ? 52 : 212 }}>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <nav className="lg:hidden flex border-t border-white/5 bg-[#050505]">
          {BOTTOM_TABS.map(tab => (
            <Link key={tab.to} to={tab.to} className={`flex-1 flex flex-col items-center gap-0.5 py-2.5 text-xs transition-colors ${isActive(tab.to) ? 'text-emerald-400' : 'text-gray-500 hover:text-gray-300'}`}>
