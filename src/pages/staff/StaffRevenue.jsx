...
function MonthlyChart({ months }) {
  if (!months || months.length === 0) {
    return <p className="text-sm text-gray-400 py-8 text-center">No monthly data available</p>;
  }
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="animate-pulse text-gray-400 text-sm">Loading revenue data...</div>
      </div>
    );
  }
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-semibold text-gray-900">By Tournament</h2>
              </div>
              <span className="text-xs text-gray-400">{tournamentBreakdown.length} tournaments</span>
            </div>
            <div className="max-h-[420px] overflow-y-auto">
              {tournamentBreakdown.length === 0 ? (
                <p className="px-5 py-8 text-sm text-gray-400 text-center">No tournament fees yet</p>
              ) : (
                    ...
                    <li key={i} className="px-5 py-3">
                      <div className="flex items-start justify-between">
                        ...
                        <div className="text-right shrink-0 ml-3">
                          <p className="text-sm font-semibold text-gray-900">{formatEGP(t.total_fee)}</p>
                          {t.paid < t.total_fee ? (
                            <p className="text-[10px] text-amber-600 mt-0.5">
                              {formatEGP(t.paid)} collected
                            </p>
                          ) : (
                            <p className="text-[10px] text-emerald-600 mt-0.5">Fully collected</p>
                          )}
                        </div>
                      </div>
                    </li>
