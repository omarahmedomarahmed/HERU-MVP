...
import {
  Trophy, Lock, Upload, Image as ImageIcon, ExternalLink, Send,
  MessageSquare, Shield, ChevronRight, AlertTriangle, Sliders,
  Radio, Clock, Check
} from 'lucide-react';
...
export default function TournamentManage() {
  ...
  useEffect(() => {
    apiCall('/auth/me').then(setUser).catch(() => navigate('/OrganizerLogin'));
    ...
    OrganizerProfile.list().then(all => {
      const p = all.find(x => x.id === session.profileId);
      setProfile(p || null);
    });
  }, [navigate]);

  const { data: tournament, isLoading } = useQuery({
    queryKey: ['tournament-manage', id],
    queryFn: () => Tournament.list().then(all => all.find(t => t.id === id) || null),
    enabled: !!id,
  });

  const { data: teams = [] } = useQuery({
    queryKey: ['tournament-manage-teams', tournament?.teams],
    queryFn: async () => {
  ...
  // Sync settings form with loaded tournament
  useEffect(() => {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries(['tournament-manage', id]),
  });
  ...
  const sendChatMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries(['tournament-manage', id]); setChatMsg(''); },
  });
  ...
  const saveSettings = async () => {
    ...
    queryClient.invalidateQueries(['tournament-manage', id]);
  ...
  // Access guard - check against user ID or profile ID (main organizer only)
  // For solo tournaments, also check created_by field as fallback
  const isMainOrg = tournament && (
    tournament.main_organizer_id === user?.id || 
    tournament.main_organizer_id === profile?.id ||
    tournament.organizer_id === user?.id ||
    tournament.organizer_id === profile?.id ||
    (tournament.tournament_type === 'solo' && tournament.created_by === user?.email) ||
    (tournament.tournament_type === 'solo' && tournament.organizer_id === profile?.id)
  );
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center">
        ...
        <FloatingPanel className="p-12 text-center">
          ...
          <button onClick={() => navigate(-1)} className="mt-4 text-gray-400 hover:text-white text-sm">← Go back</button>
        </FloatingPanel>
      </div>
    );
  }

  if (!isMainOrg && tournament && user && profile) {
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center">
        <AnimatedBackground />
        <FloatingPanel className="p-16 text-center max-w-lg mx-auto">
          ...
          <p className="text-gray-400">Only the main organizer of this tournament can access this management page.</p>
          <p className="text-gray-600 text-xs mt-2">Debug: Tournament owner {tournament.main_organizer_id || tournament.organizer_id || 'unknown'}</p>
          <GlowButton className="mt-6" onClick={() => navigate('/dashboard/organizer')}>Go to Dashboard</GlowButton>
        </FloatingPanel>
      </div>
    );
  }
          ...
          <div>
            ...
            {isShared && tournament.co_organizers?.length > 0 && (
              <div className="flex items-center gap-2 mt-3 flex-wrap">
                <span className="text-gray-500 text-xs">Co-organizers:</span>
                {tournament.co_organizers.map((co, i) => (
                  <Link key={i} to={`/organizer/${co.organizer_id}`} className="flex items-center gap-1.5 bg-zinc-800 hover:bg-zinc-700 rounded-lg px-2 py-1 transition-colors">
                    {co.brand_logo && <img src={co.brand_logo} alt="" className="w-5 h-5 rounded object-cover" />}
                    <span className="text-white text-xs font-medium">{co.brand_name}</span>
                    <span className={`text-[10px] px-1.5 rounded ${co.access_granted ? 'text-green-400' : 'text-amber-400'}`}>
                      {co.access_granted ? '✓' : '⏳'}
                    </span>
                  </Link>
                ))}
        ...
        <Tabs defaultValue="overview" className="w-full">
          ...
          <TabsContent value="teams">
            ...
            {bracketStep === 'brackets' && (
              <BracketMatchPanel tournament={tournament} teams={teams} canEdit={true} />
            )}
          </TabsContent>
