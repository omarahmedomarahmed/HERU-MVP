...
import { Briefcase, Eye, EyeOff, LogIn, AlertTriangle, DollarSign, Shield, Star } from 'lucide-react';
import { useAuth } from '@/lib/AuthContext';

const SIDE_IMG = 'https://images.unsplash.com/photo-1538481199705-c710c4e965fc?w=900&q=80';

const BULLETS = [
  { icon: DollarSign, text: '85% payout on every booking' },
  { icon: Shield,     text: 'Escrow-protected payments' },
  { icon: Star,       text: 'Get discovered by organizers' },
];
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (profile?.role !== 'service_provider') {
        await logout();
        setError('This account does not have service provider access. Please use the correct login.');
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12 lg:px-12">
        <div className="w-full max-w-sm">
          ...
          <div className="mb-8">
            ...
            <h1 className="text-3xl font-black text-white mb-1">Provider Login</h1>
            <p className="text-zinc-500 text-sm">Sign in to your HERU GIGs account</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div className="text-right -mt-2">
              <Link to="/auth/forgot-password?role=provider" className="text-xs text-cyan-400 hover:text-cyan-300 transition-colors">
                Forgot password?
              </Link>
            </div>
          ...
          <div className="mt-6 pt-6 border-t border-zinc-800 space-y-3 text-center">
            <p className="text-zinc-500 text-sm">
              ...
              <Link to="/auth/provider/register" className="text-cyan-400 hover:text-cyan-300 font-semibold transition-colors">Create Account</Link>
            </p>
