...
import {
  ...
  Send, Folder, Download, Upload, CheckCircle, Clock, AlertCircle, Shield
} from 'lucide-react';
import { format } from 'date-fns';
import { OrganizerProfile, Team, Tournament, TournamentOrder, apiCall } from '@/api/heruClient'
...
export default function CoOrganizerView() {
  ...
  useEffect(() => {
    ...
    if (session) {
      OrganizerProfile.list().then(all => {
        setProfile(all.find(x => x.id === session.profileId) || null);
      });
    }
  ...
  const { data: tournament, isLoading } = useQuery({
    queryKey: ['tournament-co', id],
    queryFn: () => Tournament.list().then(all => all.find(t => t.id === id) || null),
    enabled: !!id,
  });

  const { data: teams = [] } = useQuery({
    queryKey: ['tournament-co-teams', tournament?.teams],
    queryFn: async () => {
  ...
  const { data: tournamentOrder } = useQuery({
    queryKey: ['tournament-order-co', id],
  ...
  const sendChatMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries(['tournament-co', id]); setChatMsg(''); },
  });
  ...
  // Check co-organizer access (solo tournaments don't have co_organizers, redirect to main tournament page)
  const isMainOrganizer = tournament?.organizer_id === profile?.id;
  ...
  const filesByFolder = (folder) => {
    // Future: load from a shared file library entity
    return [];
  };
              ...
              <FloatingPanel className="p-6">
                ...
                <div className="space-y-2 max-h-80 overflow-y-auto">
                  {teams.length === 0 ? (
                    ...
                    teams.map((team, i) => (
                      <Link key={team.id} to={`/team/${team.id}`} className="flex items-center gap-3 p-2.5 bg-zinc-800/50 hover:bg-zinc-800 rounded-xl transition-colors">
